...
class QACGBertForSequenceClassification(nn.Module):
    ...
    def backward_lat(self, attention_scores):
        ...
        a_h = func_activations[layer_name_self][1][0][:,4] # b, l, l
        a_self_h = func_activations[layer_name_self][2][0][:,4]
        a_quasi_h = func_activations[layer_name_self][3][0][:,4]

        a_h = a_h.sum(dim=1).sum(dim=1)
        a_self_h = a_self_h.sum(dim=1).sum(dim=1)
        a_quasi_h = a_quasi_h.sum(dim=1).sum(dim=1)
