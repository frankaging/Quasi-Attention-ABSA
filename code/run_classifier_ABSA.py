...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    return features, unique_context_id_map
...
def Train(args):
    ...
    # training set
    train_features, unique_context_id_map = convert_examples_to_features(
    ...
    if args.eval_test:
        ...
        test_features = convert_examples_to_features(
            ...
            tokenizer, args.max_context_length,
            args.context_standalone,
            unique_context_id_map=unique_context_id_map)
    ...
    # pre-train
    logger.info("***** Performance if only BERT *****")
    # get a vanilla BERT model, for comparision
    BERT_model, _, BERT_tokenizer = \
        getModelOptimizerTokenizer(model_type="BERTPretrain",
                                   ...
                                   base_learning_rate=args.base_learning_rate,
                                   warmup_proportion=args.warmup_proportion)
    # we need to feed through the BERT once
    save_pred_file = os.path.join(args.output_dir, "test_initial.txt")
    BERT_model.to(device)
    test_loss, test_accuracy, init_grads = \
        MockTrain(args, test_dataloader, BERT_model, 
                    save_pred_file, device)
    output_log_file = os.path.join(args.output_dir, "metrics_initial.txt")
    Metrics(args, save_pred_file, output_log_file)

    diff_grads_list = []
    diff_grads = GradDiff(init_grads, init_grads)
    diff_grads_list.append(diff_grads)
    logger.info("***** Grad Diff *****")
    logger.info(diff_grads_list)
    ...
    grads_out_file = os.path.join(args.output_dir, "grads_out_init.p")
    pickle.dump( init_grads, open( grads_out_file, "wb" ) )
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        # call base train
        tr_loss, nb_tr_examples, nb_tr_steps, global_step = \
            BaseTrain(args, train_dataloader, model, optimizer, device,
                    tr_loss, nb_tr_examples, nb_tr_steps, global_step, n_gpu)
        ...
        # compare grad difference
        curr_grads = GradOnly(args, test_dataloader, model, 
                             save_pred_file, device)

        grads_out_file = os.path.join(args.output_dir, "grads_out_" + str(epoch) + ".p")
        pickle.dump( curr_grads, open( grads_out_file, "wb" ) )

        diff_grads = GradDiff(curr_grads, init_grads)
        diff_grads_list.append(diff_grads)
        logger.info("***** Grad Diff *****")
        logger.info(diff_grads_list)
